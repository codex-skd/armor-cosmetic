...
<p><strong>JAR</strong>: <code>armor_cosmetic-26.2-neoforge-1.0.22.jar</code></p>
```
...
metadata = {
    ...
    "changelog": "<h2>v1.0.22 - Port to 26.2</h2>",
    "changelogType": "html",
    "gameVersionNames": ["Client", "Server", "26.2", "NeoForge"],
    "releaseType": "release"
}

with open(f"build/libs/armor_cosmetic-26.2-neoforge-{version}.jar", "rb") as f:
    jar_data = f.read()
...
body += meta_bytes + b"\r\n"
body += f"--{boundary}\r\n".encode()
body += b'Content-Disposition: form-data; name="file"; filename="armor_cosmetic-26.2-neoforge-{version}.jar"\r\n'
