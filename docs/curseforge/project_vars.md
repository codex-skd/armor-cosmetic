...
game_versions = 9638, 9639, 16115, 10150
release_type = beta
## Proyecto
...
| Campo | Valor | Notas |
|-------|-------|-------|
| `displayName` | `Armor Cosmetic (1.1.18)` | Nombre visible: `display_name (version)` |
...
| `gameVersions` | `[9638, 9639, 10150, 16115]` | **IDs numéricos**, no nombres (la API devuelve 400 "Expected Integer but got String" si se envían strings como `"Client"`). Ver tabla de IDs abajo |
...
> Ojo: `GET https://minecraft.curseforge.com/api/game/versions` (con `X-Api-Token`) devuelve **varias entradas duplicadas** con el mismo nombre `1.21.1` pero distinto `id`/`gameVersionTypeID` (p. ej. `16498`, `16500`). Solo una es la correcta (la que ya usan los archivos existentes) — verificarlo siempre contra un archivo ya publicado antes de asumir un ID.
...
<h3>Changes</h3>
<ul>
<li><strong>Port to 1.21.1</strong>: Adapted to Minecraft 1.21.1 / NeoForge 1.21.1.0.32-beta.</li>
<li><strong>API fixes</strong>: <code>setScreenAndShow</code>, <code>gui.screen()</code>.</li>
</ul>

<hr>

<p><strong>JAR</strong>: <code>armor_cosmetic-1.21.1-neoforge-0.0.0-beta.1.jar</code></p>
```
...
$metadata = @{
    ...
    gameVersions  = @(9638, 9639, 10150, 16498)   # Client, Server, NeoForge, 1.21.1 — IDs, no nombres
    releaseType   = "release"
    changelogType = "html"
    changelog     = $changelog
}
...
$fileItem = Get-Item -Path (Resolve-Path "build/libs/armor_cosmetic-1.21.1-neoforge-$version.jar")
