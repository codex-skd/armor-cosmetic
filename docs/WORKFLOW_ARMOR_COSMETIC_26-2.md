...
|---|---|---|
| **snake_case** | `mod_id` en gradle.properties, assets/, packages Java | `armor_cosmetic` |
| **PascalCase** | Clases Java principales | `ArmorCosmetic` |
| **camelCase** | Variables, métodos, config keys | `armorCosmetic.enableFeature` |
| **Title Case** | Display name en README, CHANGELOG, docs, CurseForge | `Armor Cosmetic` |

### Ficheros de documentación

| Fichero | Formato | Ejemplo |
|---|---|---|
| WORKFLOW | `WORKFLOW_<MOD_ID>_<MC-VERSION>.md` | `WORKFLOW_ARMOR_COSMETIC_26-2.md` |
...
Reglas:
- `mod_id` en `gradle.properties` debe coincidir con el nombre del directorio del proyecto (`armor_cosmetic`)
- El display name en `README.md` y `CHANGELOG.md` debe estar en **Title Case**: `Armor Cosmetic`
- La clase Java principal: `ArmorCosmetic` (PascalCase), no `Armor_cosmetic` ni `ArmorCosmeticMod`
- Las config keys en camelCase: `armorCosmetic.enableFeature`
...
```
armor_cosmetic/              # Único repositorio Git (un solo .git/)
...
│   ├── docs/
│   └── ...
└── 26.1.2/                  # Solo existe en su rama: minecraft/26.1.2/neoforge-26.1.2.78/production
    ├── src/
    ├── docs/
    └── ...
```
...
**Reglas:**
- `armor_cosmetic/` es el repositorio Git, contiene el `.git/`
...
└── 26.2/                    # Project repo (repo root with .git)
    ...
    │   │           └── mods.toml           # Template legacy para CurseForge
    ...
    ├── docs/
    │   ├── WORKFLOW_ARMOR_COSMETIC_26-2.md  # Este documento
...
CurseForge admite **Markdown y HTML** en las descripciones y release notes. Usamos HTML porque:

- El HTML permite control preciso sobre espaciado, alineación y estructura visual
- El HTML se renderiza correctamente en el editor WYSIWYG sin escapes ni caracteres rotos
- El Markdown en CurseForge muestra caracteres escapados
...
| Ejemplo | Significado |
|---|---|
| `armor_cosmetic-26.2-neoforge-1.0.22.jar` | NeoForge 26.2, v1.0.22 |
...
|---|---|---|
| Beta | `<mc-version>-neoforge-beta.X` | `26.2-neoforge-beta.22` |
...
```bash
# Beta
git tag -a 26.2-neoforge-beta.22 -m "v0.0.0-beta.22: Port to 26.2"
git push origin 26.2-neoforge-beta.22
...
publish-public:
  ...
  script:
    ...
    - sed -i 's/^mod_curseforge_project_id=.*/mod_curseforge_project_id=/'
...
#    cp build/libs/armor_cosmetic-26.2-neoforge-<version>.jar /ruta/a/la/instancia/mods/
#    rm /ruta/a/la/instancia/mods/armor_cosmetic-26.2-neoforge-<version-anterior>.jar
```
...
#    docs/curseforge/versions/1.0.22.md
...
git tag -a 26.2-neoforge-1.0.22 -m "v1.0.22: Port to 26.2"
git push origin 26.2-neoforge-1.0.22

# 8. PREGUNTAR: "¿Subir JAR a CurseForge ahora?"
#    Solo subir si el usuario confirma.
#    El JAR está en build/libs/armor_cosmetic-26.2-neoforge-<version>.jar
...
- **README.md actualizado y en inglés**: el README debe reflejar siempre el estado actual del mod, con descripción, requisitos, instalación y enlaces. Debe estar escrito en **inglés** (en-US) por ser la puerta de entrada del proyecto desde GitHub
- **Sin residuos de mod original**: si el mod está basado en otro mod existente (fork/referencia), no debe quedar ningún rastro accidental del mod original. Revisar:
  - Nombres de paquetes (`com/oldauthor/oldmod/` → `com/skd/armorcosmetic/`)
...
| 1.2.7 | 2026-07-23 | Corrección: default=production, protected=*/main |
...
| 1.0.0 | 2026-07-22 | Versión inicial: adaptado de WORKFLOW_GENERIC.md v1.0.0 para Armor Cosmetic 26.2 |
